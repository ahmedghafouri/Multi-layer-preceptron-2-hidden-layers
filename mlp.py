...
class MLP:
    ...
    def train(self, inputs, targets, eta, niterations):
        ...
        for n in range(niterations):
            ...
            deltao =deltao*(self.outputs*(1-self.outputs)) 
            ...
            deltah1 = self.hidden1*self.beta*(1-self.hidden1)*(np.dot(deltah2[:,:-1],np.transpose(self.weights2)))



            # update the weights of the three layers: self.weights1, self.weights2 and self.weights3
            self.updatew1 = eta*(np.dot(np.transpose(inputs),deltah1[:,:-1]))+self.momentum*self.updatew1
            self.updatew2 = eta*(np.dot(np.transpose(self.hidden1),deltah2[:,:-1]))+self.momentum*self.updatew2
            self.updatew3 = eta*(np.dot(np.transpose(self.hidden2),deltao))+self.momentum*self.updatew3
    ...
    def forwardPass(self, inputs):
        ...
        self.hidden1 = 1.0/(1.0+np.exp(-self.beta*np.dot(inputs,self.weights1)))
        ...
        self.hidden2 = 1.0/(1.0+np.exp(-self.beta*np.dot(self.hidden1,self.weights2)))
        ...
        #print('self.weights2',((self.weights2).shape))
        #print('self.weights3',((self.weights3).shape))  
